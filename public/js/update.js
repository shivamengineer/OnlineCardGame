...
function updateDecks(backendDecks){
  for(const i in backendDecks){
    ...
    if(!frontendDecks[i]){
      frontendDecks[i] = new Deck(backendDeck.topCard, backendDeck.length, backendDeck.visible, backendDeck.x, backendDeck.y, backendDeck.rotation, backendDeck.currentID);
    } else {
